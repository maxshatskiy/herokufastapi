...
def run_sanity_check(test_dir):
    ...
    print('This script will perform a sanity tests to ensure your code meets the criteria in the rubric.\n')
    print('Please enter the path to the file that contains your tests cases for the GET() and POST() methods')
    ...
    if not test_functions_for_get:
        ...
        WARNING_COUNT += 1
        print(FAIL_COLOR+"No tests cases were detected for the GET() method.")
        print(FAIL_COLOR+"\nPlease make sure you have a tests case for the GET method.\
            This MUST tests both the status code as well as the contents of the request object.\n")
        ...
        if not TEST_FOR_GET_METHOD_RESPONSE_CODE:
            print(FAIL_COLOR+f"[{WARNING_COUNT}]")
            WARNING_COUNT += 1
            print(FAIL_COLOR+"Your tests case for GET() does not seem to be testing the response code.\n")
        
        if not TEST_FOR_GET_METHOD_RESPONSE_BODY:
            print(FAIL_COLOR+f"[{WARNING_COUNT}]")
            WARNING_COUNT += 1
            print(FAIL_COLOR+"Your tests case for GET() does not seem to be testing the CONTENTS of the response.\n")
    ...
    if not test_functions_for_post:
        print(FAIL_COLOR+f"[{WARNING_COUNT}]")
        WARNING_COUNT += 1
        print(FAIL_COLOR+"No tests cases were detected for the POST() method.")
        print(FAIL_COLOR+"Please make sure you have TWO tests cases for the POST() method."+
        "\nOne tests case for EACH of the possible inferences (results/outputs) of the ML model.\n")
        SANITY_TEST_PASSING = False
    else:
        if len(test_functions_for_post) == 1:
            print(f"[{WARNING_COUNT}]")
            WARNING_COUNT += 1
            print(FAIL_COLOR+"Only one tests case was detected for the POST() method.")
            print(FAIL_COLOR+"Please make sure you have two tests cases for the POST() method."+
            "\nOne tests case for EACH of the possible inferences (results/outputs) of the ML model.\n")
            SANITY_TEST_PASSING = False
        ...
        if not TEST_FOR_POST_METHOD_RESPONSE_CODE:
            print(FAIL_COLOR+f"[{WARNING_COUNT}]")
            WARNING_COUNT += 1
            print(FAIL_COLOR+"One or more of your tests cases for POST() do not seem to be testing the response code.\n")
        if not TEST_FOR_POST_METHOD_RESPONSE_BODY:
            print(FAIL_COLOR+f"[{WARNING_COUNT}]")
            WARNING_COUNT += 1
            print(FAIL_COLOR+"One or more of your tests cases for POST() do not seem to be testing the contents of the response.\n")

        if len(test_functions_for_post) >= 2 and COUNT_POST_METHOD_TEST_FOR_INFERENCE_RESULT < 2:
            print(FAIL_COLOR+f"[{WARNING_COUNT}]")
            WARNING_COUNT += 1
            print(FAIL_COLOR+"You do not seem to have TWO separate tests cases, one for each possible prediction that your model can make.")
    ...
    if SANITY_TEST_PASSING:
        print(OK_COLOR+"Your tests cases look good!")
...
if __name__ == "__main__":
    ...
    parser.add_argument('test_dir',metavar='test_dir',nargs='?',default='tests',help='Name of the directory that has tests files.')
